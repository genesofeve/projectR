...
#' @param confidence_intervals #confidence_interval is a data.frame or matrix with two columns (low, high). Genes must be rownames
#' @param pattern_name name of pattern being plotted, if NULL, defaults to 'weights'
#' @param sort Sorts genes by increasing estimate order if true
#' @param genes vector of genes that can be used to subset data
#' @param weights if provided, will generate heatmap for pattern weights
#' @param weights_clip upper limit of pattern weights
#' @param weights_vis_norm method of transforming weights to percentiles, currently only supports 'none' and 'quantiles'
