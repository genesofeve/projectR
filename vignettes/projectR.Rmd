...
#######TODO: paraphrase Asher's description here. Cite his manuscript.
Given loadings that define the weight of features (genes) in a given latent space (e.g. PCA, NMF), and the use of these patterns in samples, it is of interest to look at differential usage of these features between conditions. These conditions may be defined by user-defined annotations of cell type or by differential usage of a (projected) pattern. By looking at differences in gene expression, weighted by the loadings that define their importance in a specific latent space, a unique understanding of differential expression in that context can be gained (Baraban et al, 2021)

```
library(projectR)
projectionDriveR(cellgroup1, cellgroup2, loadings, loadingsNames = NULL,
                 alpha, feature_name, display = T, normalize_feature = T)

```

### Input Arguments
The required inputs are two feature by sample (e.g. gene by cell) matrices to be compared, the loadings that define the feature weights, and the name of the pattern (column of feature loadings).
...
\item[...]{arguments to pass to plotConfidenceIntervals}
\end{description}
...
options(width = 60)
#TODO: Change this pack to default libary before pull request
library(projectR, lib.loc = "~/R-dev/")
...
gene_ids <- rownames(conf_intervals)[idx]
...
weighted_conf_intervals <- drivers$weighted_mean_ci[rownames(conf_intervals),]
