...
\item[dataNames]{rownames (eg. gene names) of the target dataset, if different from existing rownames in data}
\item[loadingsNames]{rownames (eg. gene names) of the loadings matching the type used for GeneWeights}
...
```
library(projectR)
projectR(data, loadings,AnnotationObj = NA, IDcol = "GeneSymbol",
     NP = NA, full = FALSE)
```
