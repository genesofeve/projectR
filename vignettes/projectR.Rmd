...
# Identification of differential features through projectionDriveR.

## projectionDriveR

#######TODO: Standardize language of gene/feature/pattern. Make sure alpha is consistent. 
...
### Input Arguments
The required inputs are two feature by sample (e.g. gene by cell) matrices to be compared, the loadings that define the feature weights, and the name of the feature (column of loadings).
...
\item[alpha]{Determines the significance of the confidence interval to be calculated between the difference of means}
\item[feature_name]{the column of the loadings by which the features will be weighted}
\item[display]{Boolean. Whether or not to plot the estimates of significant features. Default = T}
\item[normalize_feature]{Boolean. Whether or not to normalize the average feature weight. Default = T}

\end{description}

## Output 
The output of \texttt{projectionDriveR} is a list of length three. \texttt{mean_bonferroni} holds the confidence intervals for the difference in means for all features, \texttt{weighted_bonferroni} holds the confidence intervals for the weighted difference in means for all features, and \texttt{significant_genes} is a vector of gene names that are significantly different at the threshold provided. 
