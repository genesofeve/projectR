...
#files <- system.file('data', package = 'projectoR')
library(projectoR)
library(CoGAPS)
data(RNAseq6l3c3t)
data(ESepiGen4c1l4)

load("data/RNAseq6l3c3t.RData")
# PCA
...
save(c.RNAseq6l3c3t,z.RNAseq6l3c3t,h.RNAseq6l3c3t,h.RNAseq6l3c3t,k.RNAseq6l3c3t,
	file="data/Patterns.RNAseq6l3c3t.RData")

load("data/ESepiGen4c1l4.RData")
names(p.ESepiGen4c1l)
# PCA
pc.ESepiGen4c1l4<-prcomp(t(p.ESepiGen4c1l4))
# CoGAPS
p.ESepiGen4c1l4<-as.matrix(p.ESepiGen4c1l4)
D <- sweep(p.ESepiGen4c1l4, 1, apply(p.ESepiGen4c1l4,1,function(x){pmin(0,min(x))}))
S <- pmax(.1*p.ESepiGen4c1l4, .1)
c.ESepiGen4c1l4<- gapsRun(D,S,nFactor=4,nEquil=5000,nSample=5000,numSnapshots=500)
# zscore for clustering 
z.ESepiGen4c1l4<-scale(p.ESepiGen4c1l4, center = TRUE, scale = TRUE) #zscore data
z.ESepiGen4c1l4[is.na(z.ESepiGen4c1l4)]<-0
# kmeans clustering 
k.ESepiGen4c1l4<-kmeans(x=z.ESepiGen4c1l4,centers=4,iter.max=10000,nstart=100,alg="Lloyd")
# hierarchical clustering 
h.ESepiGen4c1l4<- hclust(dist(z.ESepiGen4c1l4), method = "complete")

save(c.ESepiGen4c1l4,z.ESepiGen4c1l4,h.ESepiGen4c1l4,h.ESepiGen4c1l4,k.ESepiGen4c1l4,
	file="data/Patterns.ESepiGen4c1l4.RData")


```
apply(p.RNAseq6l3c3t,1,var)
cindx<-names(sort(apply(p.RNAseq6l3c3t,1,sd)))[10:dim(p.RNAseq6l3c3t)[1]]
p.RNAseq6l3c3t<-p.RNAseq6l3c3t[apply(p.RNAseq6l3c3t,1,max)>1,]

apply(p.RNAseq6l3c3t,1,max)>1

var
```{r, fig.show='hold', fig.width=3.5, fig.height=5, echo=FALSE}
#print(g_dist)
#qqnorm(glucose); qqline(glucose)
```
...
hc.RNAseq6l3c3t<-cutree(h.RNAseq6l3c3t,k=4)
class(h.RNAseq6l3c3t)
